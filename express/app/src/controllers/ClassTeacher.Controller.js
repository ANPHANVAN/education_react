...
class ClassTeacherController {
  ...
  async createClass(req, res) {
    try {
      ...
      const userAddClass = await Users.findByIdAndUpdate(teacherId, {
  ...
  async getClassTeacher(req, res) {
    try {
      ...
      const userCurrentWithClass = await Users.find({ _id: teacherId }).populate(
        'class_id',
        'grade class_name number_student school_year'
      );
  ...
  async deleteClassTeacher(req, res) {
    try {
      ...
      if (!classToDelete) {
        return res
          .status(404)
          .json({ message: 'Class not found or you do not have permission to delete this class' });
      }
  ...
  async renameClassTeacher(req, res) {
    try {
      ...
      const teacherId = req.user._id;
      const classToRename = await Class.findOneAndUpdate(
      ...
      if (!classToRename) {
        return res
          .status(404)
          .json({ message: 'Class not found or you do not have permission to rename this class' });
      }
  ...
  async getClassDetailsVideo(req, res) {
    try {
      ...
      console.error(
        'Failure to into route /class-teacher/api/classroom-details/:classId/video  :',
        err
      );
      res.status(500).send('Internal Server Error');
    }
  ...
  async getClassDetailsAnnounce(req, res) {
    try {
      ...
      console.error(
        'Failure to find class info in route /class-teacher/api/classroom-details/:classId/announce'
      );
      res.status(500).send('Internal Server Error');
    }
  }
  ...
  async addStudentToClass(req, res) {
    try {
      ...
      const userStudent = await Users.findOneAndUpdate(
        { email: studentEmail },
        {
          $push: {
            class_id: classId,
  ...
  async deleteStudentFromClass(req, res) {
    try {
      ...
      const userStudent = await Users.findOneAndUpdate(
        { _id: studentInfo.student_user_id },
        {
          $pull: {
            class_id: classId,
          },
      ...
      const classDetails = await Class.findByIdAndUpdate(
        classId,
        { $pull: { students: { student_id: studentId } }, $inc: { number_student: -1 } },
        { new: true }
      );
