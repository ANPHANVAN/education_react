...
class ClassTeacherController {
    ...
    async essayClassDetail(req,res) {
        try {
            ...
            console.error("Failure when render engine ejs with route /essay-teacher/essay-class-detail?essay-id=:essayId&class_id=classId:", err)
            res.status(500).send("Internal Server Error")
        }
    }
    ...
    async getMyEssay(req,res) {
        try {
            const userId = req.user._id
            const essayList = await Essay.find({ teacher_owner_id:userId })
            .populate({
                path: 'class',
                select: 'class_name'
            })
    ...
    async getEssayDetail(req, res) {
        try {
            ...
            if (!essayList) {
                return res.status(404).json({ message: 'essayList not found or you do not have permission to access it.' });
            }
    ...
    async deleteEssay(req, res) {
        try {
            ...
            const deletedEssay = await Essay.findOneAndDelete({ _id: essayId, teacher_owner_id: teacherId });

            if (!deletedEssay) {
                return res.status(404).json({ message: 'Essay not found or you do not have permission to delete it.' });
            }
    ...
    async getEssayClassDetail(req,res) {
        try {
            ...
            const classInfo = await Class.findById(classId,{announcement:0})
                .populate({
                    path:'students',
                    populate: {
            ...
            console.error("Failure with route // [GET] /test-teacher/api/test-class-detail?test-id=:essayId&class_id=classId", err)
            res.status(500).send("Internal Server Error")
        }
    }
